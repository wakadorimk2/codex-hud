...
通常起動時は、HUDプロセスがタスクトレイへアイコンを登録します。
...
ツールチップには、HUDの起動状態と`NeedsAttention`、`Running`、`Idle`の件数を表示します。

ツールチップには、セッションID、prompt、command、cwd、生のHook payloadを表示しません。

HUDを非表示にしても、Named Pipe、Session State Store、Hook受信は継続します。

`終了`を選ぶとHUDプロセスを終了します。

次の`SessionStart` Hookで、HUDは既存の起動処理により再起動します。

`--hook`起動時は、タスクトレイアイコンを登録しません。
