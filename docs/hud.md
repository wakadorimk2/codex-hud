...
- 一時的な通知イベントだけを正にしません。
- 通常時は作業を妨げません。
- 要対応状態は状態遷移として時間経過だけで解除しません。ただし、時刻がある一覧項目は1時間で整理します。
...
| 状態 | 既定の表示 | 音 | 自動消去 |
| --- | --- | --- | --- |
| `Running` | Passive modeで低い注意度にします | なし | 状態が変わるまで保持し、最終活動から1時間で一覧整理します |
| `Completed` | Passive modeで低い注意度にします | なし | 最終活動から1時間で一覧整理します |
| `WaitingForUser` | 持続的な視覚強調を行います | 将来、状態遷移時の補助音を検討します | 状態遷移では時間で解除せず、最終活動から1時間で一覧整理します |
| `WaitingForApproval` | `WaitingForUser`より高い視覚強調を行います | 将来、状態遷移時の補助音を検討します | 状態遷移では時間で解除せず、最終活動から1時間で一覧整理します |
| `Error` | 最も高い視覚強調を行います | 将来、状態遷移時の補助音を検討します | 明示的な回復または新しいturnで解除し、最終活動から1時間で一覧整理します |
...
カタログに存在しないセッションは、最終Hook観測から1時間でHUD一覧から削除します。

カタログに存在するセッションは、最終Hook観測またはカタログ最終更新から1時間でHUD一覧から削除します。
